 ...
 */
class CloudflareDNSUpdater extends EventEmitter {
  ...
  startWatcher() {
    ...
    const _check = () => {
      this.config.verbose && console.log(`${(new Date).toISOString()}: checking for ip change, current is`, this._currentIP);
      if (pending) return;
      pending = true;
      return this.checkIP()
        .then((ip) => {
          ...
          if (this._currentIP === ip)
            return void this.config.verbose && console.log(`No change detected.`);
